...
credentialTypeId: text("credential_type_id",
                    { enum: ["com.emvco.dpc.card", "av"] }).notNull(), // new
...
Migration `0001`. SQLite cannot relax `NOT NULL` in place, so drizzle-kit emits a
table recreate. The generated `INSERT … SELECT` must be hand-edited to supply
`'com.emvco.dpc.card'` for the new column, because drizzle-kit cannot know the
backfill value and a `NOT NULL` column with no default fails against a non-empty
table. A test asserts an existing row survives the migration with the DPC type.
...
11 foundry-client + 31 ui). This design adds roughly **24**, all in
`apps/bank`. Measure the real number; do not restate the projection.
